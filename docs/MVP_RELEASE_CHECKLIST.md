...
- [ ] Backend: `backend/.env` (or env vars) set with `SECRET_KEY`, `DATABASE_URL`, `REDIS_URL`; optional `OPENAI_API_KEY`, `OPENROUTER_API_KEY`
...
- [ ] Summarize / Extract keywords on a note (if OpenAI configured); 429 when over quota
